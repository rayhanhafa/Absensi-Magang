<x-app-layout title="Dashboard Admin">
    <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 mt-1">Ringkasan aktivitas absensi hari ini.</p>
        </div>
    </div>

    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        <x-stat-card label="Peserta Aktif" :value="$totalInternAktif" color="indigo" icon="users" />
        <x-stat-card label="Total Mentor" :value="$totalMentor" color="blue" icon="user-group" />
        ...
        <x-stat-card label="Izin Hari Ini" :value="$izinHariIni" color="blue" icon="document-text" />
        <x-stat-card label="Sakit Hari Ini" :value="$sakitHariIni" color="purple" icon="shield-check" />
        <x-stat-card label="Alpa Hari Ini" :value="$alpaHariIni" color="red" icon="x-circle" />
    </div>

    <x-card title="Absensi Hari Ini" noPadding class="shadow-sm border-slate-200/80">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($absensiHariIni as $absensi)
                        <tr>
                            ...
                            <td class="px-5 py-3 text-slate-600">{{ $absensi->intern->instansi ?? '-' }}</td>
